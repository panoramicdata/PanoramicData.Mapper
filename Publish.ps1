...
# Write-Host cannot be captured, suppressed or redirected, and does nothing in a host that has no
# console. Write-Information carries the same text through the information stream instead; the
# explicit -InformationAction keeps it visible without the caller having to set $InformationPreference.
function Write-Info {
...
Write-Info "Waiting for the release run for $version..."
...
Write-Info "Run: https://github.com/$repoFullName/actions/runs/$runId"
...
if ($runExitCode -ne 0) {
	...
	Write-Info "  gh run rerun $runId --repo $repoFullName --failed"
	exit 1
}

Write-Info "Package $version published."